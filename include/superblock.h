...
/** \brief capacity of the deleted inode queue \anchor DELETED_QUEUE_SIZE */
...
#define REF_CACHE_SIZE 60
    ...
    struct SOSuperblock
    {
        ...
        /** \brief number of free inodes \anchor ifree */
        uint16_t ifree;
         ...
         * - A 1 means the corresponding inode is free, and 0 it is in-use.
         * \anchor ibitmap
         */
        uint32_t ibitmap[MAX_INODES/32];

        /** \brief queue of deleted/hidden inodes, managed in a circular FIFO \anchor iqueue */
        uint16_t iqueue[DELETED_QUEUE_SIZE];

        /** \brief index of oldest reference in \c iqueue \anchor iqhead */
        uint8_t iqhead;

        /** \brief number of elements in \c iqueue \anchor iqcount */
        uint8_t iqcount;
        ...
        /** \brief number of free data blocks \anchor dbfree */
        uint32_t dbfree;
        ...
        /** \brief bit index where search for free data blocks should start from \anchor rbm_idx */
        uint32_t rbm_idx;
        ...
        /** \brief retrieval cache of references to free data blocks \anchor retrieval_cache */
        ReferenceCache retrieval_cache;

        /** \brief insertion cache of references to free data blocks \anchor insertion_cache */
